...
def _load_optimizer(config: Dict[str, Any]) -> Optional[Callable]:
    ...
    try:
        mod = importlib.import_module(f"decidra.strategy.backtest.optimizers.{opt_name}")
...
class BaseEngine(ABC):
    ...
    def __init__(self, config: dict):
        ...
        self._f3_warned_symbols: set[str] = set()  # F3 告警去重：每标的只警一次
    ...
    def run_backtest(
        ...
        if not isinstance(signal_map, dict):
            raise ValueError(
                "SignalEngine.generate() must return Dict[str, pd.Series], "
        ...
        benchmark_metadata = {}
        # 外部基准（可选）：config["benchmark"] 为一个标的代码（如指数 000300），
        # 经同一 loader 取其收盘收益作基准；缺省用策略标的等权收益（ret_df 均值）。
        bench_ticker = config.get("benchmark")
        if bench_ticker:
            try:
                bench_frames = loader.fetch(
                    [bench_ticker],
                    config.get("start_date", ""),
                    config.get("end_date", ""),
                    interval=interval,
                ...
                if bench_df is not None and not bench_df.empty:
                    bench_ret = bench_df["close"].pct_change().reindex(dates).fillna(0.0)
                    benchmark_metadata = {"benchmark_ticker": bench_ticker}
            except Exception as exc:  # noqa: BLE001 - 基准非关键，取数失败回退等权默认
                logger.warning("基准 %s 取数失败，回退等权默认基准: %s", bench_ticker, exc)
        ...
        # 7. Validation（可选，由 config["validation"] 触发：monte_carlo/bootstrap/walk_forward）
        if config.get("validation"):
            from .validation import run_validation, write_validation_json
    ...
    def _plan_open_order(
        ...
        raw_size = self._calc_raw_size(symbol, target_notional, price)
        size = self.round_size(raw_size, price)
        if size <= 0:
            # F3: 本应下单（目标名义 > 0）却因整手取整归零——资金过小或股价过高，
            # 静默丢单会让权益曲线平坦无解释，此处告警（每标的仅一次，避免逐根刷屏）。
            if raw_size > 0 and symbol not in self._f3_warned_symbols:
                self._f3_warned_symbols.add(symbol)
                logger.warning(
                    "F3 %s：目标名义 %.2f 经整手取整后为 0 手（价 %.2f），已跳过下单",
                    symbol,
                    target_notional,
                    price,
                )
